...
/*   Updated: 2024/08/15 16:05:54 by dcsicsak         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 */
static int	resolve_special_paths(t_command *cmd, char **target_path)
{
	char	*curpath;
	...
	{
		curpath = getenv("OLDPWD");
		// If OLDPWD is not set, display an error message.
		if (curpath == NULL)
			return (ft_putstr_fd("minishell: cd: OLDPWD not set\n",
					STDERR_FILENO), 1);
		printf("%s\n", curpath); // Print the old path when using `cd -`.
	}
	// Check if no arguments are passed, which means the user wants to go to HOME.
 ...
 */
static int	resolve_target_path(t_command *cmd, char **target_path)
{
	char	*curpath;
	char	*resolved_path;
	...
	if (resolve_special_paths(cmd, target_path) == 0 && *target_path != NULL)
		return (0);
	...
	{
		resolved_path = resolve_cdpath_if_needed(curpath);
 ...
 */
static int	change_directory(const char *curpath)
{
	char	cwd[PATH_MAX];
	...
	return (update_directory_env(cwd));
}
...
int	builtin_cd(t_command *cmd)
{
	...
	result = resolve_target_path(cmd, &target_path);
